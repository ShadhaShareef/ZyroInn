...
$ownerNavItems = [
    ['label' => 'Dashboard',       'href' => BASE_URL . '/owner/index.php?route=dashboard',        'icon' => svg_icon('dashboard', 'w-5 h-5'), 'active' => $route === 'dashboard'],
    ['label' => 'Rate & Inventory','href' => BASE_URL . '/owner/index.php?route=inventory',        'icon' => svg_icon('inventory', 'w-5 h-5'), 'active' => $route === 'inventory'],
    ['label' => 'Property Features','href' => BASE_URL . '/owner/index.php?route=property-features','icon' => svg_icon('features', 'w-5 h-5'), 'active' => $route === 'property-features'],
    ['label' => 'Alerts',          'href' => BASE_URL . '/owner/index.php?route=alerts',            'icon' => svg_icon('alerts', 'w-5 h-5'), 'active' => $route === 'alerts'],
    ['label' => 'Staff Scheduling','href' => BASE_URL . '/owner/index.php?route=staff-scheduling',  'icon' => svg_icon('staff', 'w-5 h-5'), 'active' => $route === 'staff-scheduling'],
    ['label' => 'Expenses',        'href' => BASE_URL . '/owner/index.php?route=expenses',          'icon' => svg_icon('expenses', 'w-5 h-5'), 'active' => $route === 'expenses'],
    ['label' => 'Reports',         'href' => BASE_URL . '/owner/index.php?route=reports',           'icon' => svg_icon('reports', 'w-5 h-5'), 'active' => $route === 'reports'],
    ['label' => 'Property Settings','href' => BASE_URL . '/owner/index.php?route=property-settings','icon' => svg_icon('settings', 'w-5 h-5'), 'active' => $route === 'property-settings'],
];
$userInitials = strtoupper(substr($_SESSION['first_name'] ?? 'O', 0, 1) . substr($_SESSION['last_name'] ?? 'U', 0, 1));
$userName = htmlspecialchars(($_SESSION['first_name'] ?? 'Owner') . ' ' . ($_SESSION['last_name'] ?? 'User'));
?>
<!DOCTYPE html>
    ...
    <aside class="hidden md:flex md:flex-col bg-white border-r border-neutral-200 shadow-sm transition-all duration-300" :class="sidebarOpen ? 'w-64' : 'w-16'">
      <div class="flex h-16 items-center justify-between px-4 border-b border-neutral-200">
        <div x-show="sidebarOpen" class="flex items-center gap-2">
          ...
          <span class="rounded-pill bg-brand-50 px-1.5 py-0.5 text-[9px] font-bold text-brand-700 uppercase tracking-wider border border-brand-100">Owner</span>
        </div>
      ...
      <div class="border-t border-neutral-200 p-4" x-show="sidebarOpen">
        <div class="flex items-center gap-2 rounded-pill bg-neutral-100 px-3 py-2 text-xs">
          <span class="font-semibold text-neutral-500">Property</span>
          <select onchange="window.location.href='<?= BASE_URL ?>/owner/index.php?route=<?= htmlspecialchars($route) ?>&property_id=' + this.value" class="bg-transparent font-semibold text-brand-600 outline-none flex-1 min-w-0">
      ...
      <div class="border-t border-neutral-200 p-4" x-show="sidebarOpen">
        <div class="flex items-center gap-3">
          ...
          <div class="text-xs">
            ...
            <p class="text-neutral-500">Property Owner</p>
          </div>
        </div>
        <a href="<?= BASE_URL ?>/owner/index.php?route=logout" class="mt-3 flex items-center gap-2 rounded-2xl px-3 py-2 text-xs font-semibold text-neutral-500 hover:bg-rose-50 hover:text-rose-600 transition">
          Log Out
        </a>
      </div>
    </aside>
      ...
      <div class="absolute left-0 top-0 bottom-0 w-72 bg-white shadow-xl" @click.away="mobileNavOpen = false">
        <div class="flex h-16 items-center justify-between px-4 border-b border-neutral-200">
          <div class="flex items-center gap-2">
            <span class="text-xl font-bold bg-gradient-to-r from-brand-600 to-brand-400 bg-clip-text text-transparent">ZyroInn</span>
            <span class="rounded-pill bg-brand-50 px-1.5 py-0.5 text-[9px] font-bold text-brand-700 uppercase tracking-wider">Owner</span>
          </div>
      ...
      <header class="bg-white border-b border-neutral-200 sticky top-0 z-30 shadow-sm">
        <div class="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div class="hidden md:flex items-center">
            <h2 class="text-lg font-semibold text-brand-900"><?= htmlspecialchars($title) ?></h2>
          </div>
          <div class="flex items-center gap-4">
            ...
            <?php endif; ?>
            <a href="<?= BASE_URL ?>/owner/index.php?route=logout" class="text-xs font-semibold text-neutral-500 hover:text-rose-600 transition">Log Out</a>
            <div class="flex h-9 w-9 items-center justify-center rounded-full bg-brand-100 text-brand-700 font-bold text-sm">
              <?= $userInitials ?>
            </div>
          </div>
